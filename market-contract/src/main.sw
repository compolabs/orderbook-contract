...
fn get_asset_type(asset_id: AssetId) -> AssetType {
    require(asset_id == BASE_ASSET || asset_id == QUOTE_ASSET, AssetError::InvalidAsset);
    if asset_id == BASE_ASSET { AssetType::Base } else { AssetType::Quote }
}
...
impl Market for Contract {
    ...
    #[storage(read, write)]
    fn open_order(base_token: AssetId, base_size: I64, base_price: u64) -> b256 {
        ...
        if base_size.negative {
            ...
            require(
                msg_amount() == base_size_to_quote_amount(base_size.value, BASE_ASSET_DECIMALS, base_price),
                "Error::BadValue",
            );
            require(msg_asset_id() == QUOTE_ASSET, "Error::BadAsset");
        }
        ...
        let order_id = gen_order_id(user, base_token, base_price);
        ...
        if order.is_some() {
            ...
            // log
            if refund_0 > 0 {
                let asset_type = get_asset_type(asset_id_0);
                account.locked.credit(refund_0, asset_type);
                account.liquid.debit(refund_0, asset_type);
            }
            if refund_1 > 0 {
                let asset_type = get_asset_type(asset_id_1);
                account.locked.credit(refund_1, asset_type);
                account.liquid.debit(refund_1, asset_type);
            }
        } else {
            let order = Order {
                id: order_id,
    ...
    //     //достаем по id
    //     //проверяем что ордер принадлежит тому кто вызывает контракт
    //     //удаляем ордер записать в сторадж что его нет
    //     //перевести деньги
    ...
    // #[storage(read, write)]
    // fn match(sell_order_id: b256, buy_order_id: b256);
...
fn add_order_internal(order: Order) {
    ...
    storage.user_orders.get(order.trader).push(order.id);
    storage
        .user_order_indexes
        .get(order.trader)
        .insert(order.id, storage.user_orders.get(order.trader).len()); // pos + 1 indexed
}
...
fn remove_update_order_internal(order: Order, base_size: I64) {
    if (order.base_size == base_size.flip()) {
        let pos_id = storage.user_order_indexes.get(order.trader).get(order.id).read() - 1; // pos + 1 indexed
        assert(storage.user_order_indexes.get(order.trader).remove(order.id));
        assert(storage.user_orders.get(order.trader).swap_remove(pos_id) == order.id);
