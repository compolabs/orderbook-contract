...
#[tokio::test]
async fn open_base_token_order_cancel_test() -> anyhow::Result<()> {
    //--------------- WALLETS ---------------
    ...
    let response = orderbook.market_exists(btc.asset_id).await?;
    assert!(response.value);

    let response = orderbook.orders_by_trader(admin.address()).await?;
    ...
    let amount_btc = base_size_sell1.unsigned_abs() as u64;

    // Mint BTC
    btc.mint(admin.address().into(), amount_btc).await?;
    let balance = admin.get_asset_balance(&btc.asset_id).await?;
    ...
    assert_eq!(admin.get_asset_balance(&btc.asset_id).await?, 0);

    let response = orderbook.orders_by_trader(admin.address()).await?;

    assert_eq!(1, response.value.len());

    let order_id = response.value.first().unwrap();
    let response = orderbook.order_by_id(order_id).await?;
    ...
    // Mint USDC
    usdc.mint(admin.address().into(), amount_usdc).await?;

    let balance = admin.get_asset_balance(&usdc.asset_id).await?;
    ...
    orderbook
        .open_order(btc.asset_id, base_size_buy2, base_price)
        .await?;
...
#[tokio::test]
async fn open_quote_token_order_cancel_by_reverse_order_test() -> anyhow::Result<()> {
    //--------------- WALLETS ---------------
    let wallets_config = WalletsConfig::new(Some(5), Some(1), Some(1_000_000_000));
    let wallets = launch_custom_provider_and_get_wallets(wallets_config, None, None).await?;
    let admin = &wallets[0];
    ...
    let base_price = price * 10u64.pow(PRICE_DECIMALS);
    let base_size_buy1 = btc.parse_units(btcv) as i64;
    let amount_btc = base_size_buy1.unsigned_abs() as u64;
    ...
    assert_eq!(admin.get_asset_balance(&usdc.asset_id).await?, amount_usdc);
    assert_eq!(admin.get_asset_balance(&btc.asset_id).await?, amount_btc);
    ...
    assert_eq!(admin.get_asset_balance(&usdc.asset_id).await?, 0);
...
#[tokio::test]
async fn match_orders_test() -> anyhow::Result<()> {
    //--------------- WALLETS ---------------
    let wallets_config = WalletsConfig::new(Some(5), Some(1), Some(1_000_000_000));
    let wallets = launch_custom_provider_and_get_wallets(wallets_config, None, None).await?;
    let admin = &wallets[0];
    ...
    usdc.mint(user1.address().into(), amount_usdc).await?;
    btc.mint(user2.address().into(), amount_btc).await?;

    assert_eq!(user1.get_asset_balance(&usdc.asset_id).await?, amount_usdc);
    assert_eq!(user2.get_asset_balance(&btc.asset_id).await?, amount_btc);
    ...
    assert_eq!(user1.get_asset_balance(&usdc.asset_id).await?, 0);

    let response = orderbook.orders_by_trader(user1.address()).await?;

    assert_eq!(1, response.value.len());

    let order_id_1 = response.value.first().unwrap();
    let response = orderbook.order_by_id(order_id_1).await?;
    ...
    assert_eq!(user2.get_asset_balance(&btc.asset_id).await?, 0);

    let response = orderbook.orders_by_trader(user2.address()).await?;

    assert_eq!(1, response.value.len());

    let order_id_2 = response.value.first().unwrap();
    let response = orderbook.order_by_id(order_id_2).await?;
    ...
    // Match orders
    orderbook.match_orders(order_id_2, order_id_1).await?;
    ...
    assert_eq!(user2.get_asset_balance(&usdc.asset_id).await?, amount_usdc);
    assert_eq!(user1.get_asset_balance(&btc.asset_id).await?, amount_btc);
