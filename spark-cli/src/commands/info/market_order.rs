...
#[derive(Args, Clone)]
#[command(about = "Query the market for information about a specific open market order")]
pub(crate) struct MarketOrderCommand {
...
impl MarketOrderCommand {
    pub(crate) async fn run(&self) -> anyhow::Result<()> {
        ...
        let order = contract.market_order(order_id).await?.value;

        match order {
            Some(order) => {
                println!("Is market order: {}", order);
            }
