...
#[derive(Args, Clone)]
#[command(about = "Deploys the psycho/usdc market proxy to a network")]
pub(crate) struct DeployPsychoUsdcProxyCommand {}

impl DeployPsychoUsdcProxyCommand {
    pub(crate) async fn run(&self) -> anyhow::Result<()> {
        ...
        let base_asset = AssetId::from_str(&psycho).unwrap();
        ...
        let min_price = 1_000; // 0.000001 USDC
        let min_size = 400_000; // 0.0004 PSYCHO
