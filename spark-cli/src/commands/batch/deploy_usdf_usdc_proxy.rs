...
#[derive(Args, Clone)]
#[command(about = "Deploys the usdf/usdc market proxy to a network")]
pub(crate) struct DeployUsdfUsdcProxyCommand {}

impl DeployUsdfUsdcProxyCommand {
    pub(crate) async fn run(&self) -> anyhow::Result<()> {
        ...
        let base_asset = AssetId::from_str(&usdf).unwrap();
        ...
        let min_price = 1_000_000; // 0.001 USDC
        let min_size = 1_000_000; // 0.001 USDF
